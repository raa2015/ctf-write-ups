...
async function refreshState() {
  try {
    walletState = await sendMsg({ action: "getState" });
  } catch {
    walletState = null;
  }
  return walletState;
...
route("/import/mnemonic", () => {
  ...
  on("btn-import-mnemonic", async () => {
    ...
    try {
      await sendMsg({ action: "importMnemonic", mnemonic: phrase, name: name || void 0 });
...
route("/import/key", () => {
  ...
  on("btn-import-key", async () => {
    ...
    try {
      await sendMsg({ action: "importPrivateKey", privateKey: key, name: name || void 0 });
      toast("Wallet imported!");
      navigate("/wallet");
    } catch (e) {
      toast(e.message, true);
...
route("/confirm", () => {
  ...
  container.innerHTML = `
    ...
    <div class="flex flex-col gap-3 mt-auto">
      ${p.type === "eth_sendTransaction" ? `<button disabled class="w-full h-14 border-4 border-black bg-neo-bg font-black text-[11px] uppercase tracking-widest text-black/30 cursor-not-allowed">Sending Disabled</button>` : btnPrimary("Sign", "btn-confirm")}
...
route("/settings", () => {
  ...
  const connectedRows = connected.length > 0 ? connected.map((site) => `
      ...
      `).join("") : `<div class="text-[11px] font-bold opacity-40 uppercase tracking-wide">No connected sites</div>`;
  const privilegedRows = privileged.length > 0 ? privileged.map((site) => `
        <div class="border-4 border-black bg-neo-muted/30 px-3 py-2 shadow-neo-sm">
          <span class="font-bold text-[11px] truncate">${site}</span>
        </div>
      `).join("") : `<div class="text-[11px] font-bold opacity-40 uppercase tracking-wide">None</div>`;
  return `
    <div class="flex-1 flex flex-col">
      <div class="flex-1 p-5">
        ${backBtn("WALLET", "/wallet")}
